...
import { CalendarDays, ChevronLeft, CloudSun, Leaf, MapPin } from "lucide-react";
...
import { formatFestivalDates, getFestivalStatus, getFestivalsByRegion } from "@/lib/data/festivals";
import { useWeather } from "@/lib/hooks/useWeather";
import { cn } from "@/lib/utils/cn";
import { formatMonthDay } from "@/lib/utils/date";
import { RegionWithStatus } from "@/types/region";

interface Props { region: RegionWithStatus }
    ...
    <div className="mx-auto max-w-3xl space-y-6 px-4 py-8">
      ...
      <div className="card space-y-4 p-5">
        ...
        <div className="grid grid-cols-3 gap-3 text-center">
          {[
            { label: "개화", date: formatMonthDay(region.bloom), active: region.status !== "before" },
            { label: "만개", date: formatMonthDay(region.peak),  active: region.status === "peak" || region.status === "falling" || region.status === "done" },
        ...
        <DDay status={region.status} daysUntilBloom={region.daysUntilBloom} daysUntilPeak={region.daysUntilPeak} daysUntilFall={region.daysUntilFall} />
      </div>
        ...
        <div className="card p-5">
          ...
          {isLoading ? (
            <div className="flex justify-center py-4">
              <div className="h-6 w-6 animate-spin rounded-full border-2 border-sakura-800 border-t-[#ff4da6]" />
            </div>
