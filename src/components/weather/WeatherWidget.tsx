...
export function WeatherWidget({ weather, isLoading }: Props) {
  ...
  return (
    <div className="space-y-3">
      {/* 현재 날씨 */}
      <div className="bg-sakura-800 flex items-center justify-between rounded-xl px-4 py-3">
        ...
        <div className="text-text-muted space-y-1 text-right text-xs">
          <p className="flex items-center justify-end gap-1">
            <Wind size={11} /> {Math.round(current.windspeed)} km/h
          </p>
          <p className="flex items-center justify-end gap-1">
            <CloudRain size={11} /> {current.precipitation.toFixed(1)} mm
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-2 sm:grid-cols-7">
        {daily.slice(0, 7).map((d, i) => (
          <div
            key={d.date}
            className="bg-sakura-900 flex flex-col items-center gap-1 rounded-xl border border-[rgba(255,77,166,0.2)] px-2 py-2 text-center"
          >
            ...
            <p className="text-text-dim text-xs">{Math.round(d.tempMin)}°</p>
